...
void ComputePatches(WFObject *obj) {
	...
	*/
	for(int i = 0; i < faces->size(); i++) {
		...
		desc.OriginalQuad[0] = vertices->at(current->GetVertexIndex(0));
		desc.OriginalQuad[1] = vertices->at(current->GetVertexIndex(1));
		desc.OriginalQuad[2] = vertices->at(current->GetVertexIndex(2));
		desc.OriginalQuad[3] = vertices->at(current->GetVertexIndex(3));
		...
		/* Calcoliamo i centrali dei lati della patch */

		desc.ControlPoints[1] = lerp(desc.OriginalQuad[0], desc.OriginalQuad[1], 1.0f / 3.0f);
		desc.ControlPoints[2] = lerp(desc.OriginalQuad[0], desc.OriginalQuad[1], 2.0f / 3.0f);

		desc.ControlPoints[7] = lerp(desc.OriginalQuad[1], desc.OriginalQuad[2], 1.0f / 3.0f);
		desc.ControlPoints[11] = lerp(desc.OriginalQuad[1], desc.OriginalQuad[2], 2.0f / 3.0f);

		desc.ControlPoints[14] = lerp(desc.OriginalQuad[2], desc.OriginalQuad[3], 1.0f / 3.0f);
		desc.ControlPoints[13] = lerp(desc.OriginalQuad[2], desc.OriginalQuad[3], 2.0f / 3.0f);

		desc.ControlPoints[8] = lerp(desc.OriginalQuad[3], desc.OriginalQuad[0], 1.0f / 3.0f);
		desc.ControlPoints[4] = lerp(desc.OriginalQuad[3], desc.OriginalQuad[0], 2.0f / 3.0f);
