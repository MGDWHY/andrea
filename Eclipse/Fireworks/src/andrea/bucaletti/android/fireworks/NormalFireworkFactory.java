...
import andrea.bucaletti.android.fireworks.SparkManager.SparkDescriptor;
...
public class NormalFireworkFactory extends FireworkFactory {
	...
	public ByteBuffer generate(Params params) {
		...
		int numSparks = (int)Math.round(nsFactor * random() * (MAX_FIREWORK_SPARKS - MIN_FIREWORK_SPARKS) + MIN_FIREWORK_SPARKS);
		...
		synchronized (this) {	
			...
			for(int i = 0; i < numSparks; i++) {			
				...
				desc.vx = FloatMath.cos(angle) * velocity * crazyness;
				desc.vy = FloatMath.sin(angle) * velocity * crazyness;
