 ...
 * @author Andrea
 */
public class CFBDecipher extends KeyFunction<Integer> {
  
    private byte[] iv;
    
    public CFBDecipher(InputStream in) { super(in); }
    ...
    public byte[] execute() {
        ...
        while((dataIn = read(4)) != null) {
            ...
            for(int i = 0; i < len; i++) { // decryption
                ...
                dataOut[i] = (byte)(dataIn[i] ^ r ^ register[i]);
            }
